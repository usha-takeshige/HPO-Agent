...
| `dummy_eval_fn` | `eval_fn: (params: dict) -> float` 実装 | 固定スコア（0.85）を返す。`time.sleep(0.001)` を入れ `eval_duration > 0` を保証 | A-1, A-2, A-3 |
| `DeterministicEvalFn` | eval 関数 | `-(num_leaves - 64)**2 / 10000 + 0.9` のような2次関数でスコアを確定的に返す | A-3（HPO 性質テスト） |
...
from hpo_agent.models import HPOConfig, ParamSpec, ParamSpace, TrialRecord
...
| CMP-05 | `HPOResult.trials_df` | 必須カラムをすべて含む | `n_trials=5` で実行 | `trial_id`, `score`, `tool_used`, `timestamp`, `eval_duration`, `algo_duration`, `reasoning` カラムが存在する。`params` 列は存在せず、代わりに各パラメータ（例: `num_leaves`, `learning_rate`）が個別カラムとして展開されている |
...
```python
# test/check_behavior.py
"""
振る舞いテスト：グラフ出力による定性確認スクリプト。
実行方法: uv run python test/check_behavior.py
"""
...
# サブカテゴリ別実行
uv run pytest test/ -m component
uv run pytest test/ -m agent
uv run pytest test/ -m hpo

# 振る舞いテスト（手動確認）
uv run python test/check_behavior.py
```

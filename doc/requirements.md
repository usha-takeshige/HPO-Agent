...
- 含む内容：
  ...
  - `ChangeSearchSpaceTool` 実行時：変更前後の探索空間（変更時点の試行数を含む）
...
- 含む内容：
  ...
  - スコア推移の考察
  - 使用ツールと試行回数の内訳
  - 探索空間変更履歴（変更があった場合）：変更回数・各変更時点の試行数・変更前後の空間
...
- **ツール選択時のログ**（`logging.INFO`）：スーパーバイザーが LLM の判断でツールを選択した直後に、ツール名と計画試行回数を出力する
- **ツール実行開始時のログ**（`logging.INFO`）：ツール実行開始直前に、確定試行回数（`remaining_trials` による上限適用後）・要求試行回数・残り試行数を出力する

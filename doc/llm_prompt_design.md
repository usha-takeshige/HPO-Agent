...
| change_search_space | 探索空間の動的変更ツール（狭め・拡大）の用途・注意点を明示する |
...
| LLM 自動生成温度設定 | `_generate_param_space()` は `temperature=0`（決定的な探索空間を生成）とする | `HPOAgent._generate_param_space()` |
...
`HPOAgent.run()` が呼ばれた際、`param_space=None` の場合に HPO 開始前に一度だけ呼ばれる。

### 9-2. 入力情報

| 情報 | 取得方法 | 用途 |
|------|---------|------|
| モデルクラス名 | `type(config.model).__name__` | どの ML フレームワークかを LLM が判断 |
| `eval_fn` ソースコード | `inspect.getsource(config.eval_fn)` | モデルのタスク・データ構造を LLM が推定 |
| 試行回数 | `config.n_trials` | 試行数に見合ったパラメータ数を設計 |

※ `inspect.getsource()` がラムダ・動的定義関数で失敗した場合は `"(ソース取得不可)"` を渡す。

### 9-3. 出力スキーマ（structured output）

LangChain の `llm.with_structured_output(ParamSpaceSchema)` を使用する。
...
class ParamSpaceSchema(BaseModel):
    specs: list[ParamSpecSchema]
```

`ParamSpaceSchema.to_param_space()` で frozen dataclass の `ParamSpace` に変換する。

### 9-4. プロンプト定数

`PARAM_SPACE_GENERATION_PROMPT`（`src/hpo_agent/prompts.py`）にテンプレートを定義し、
`format()` でモデル名・試行回数・`eval_fn` ソースを埋め込む。

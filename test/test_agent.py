...
# ---------------------------------------------------------------------------
# AGT-14〜17: ChangeSearchSpaceTool 連携テスト
# ---------------------------------------------------------------------------
...
class TestChangeSearchSpaceIntegration:
    def test_change_search_space_tool_selectable(
        ...
        change_args = json.dumps([{"name": "num_leaves", "low": 30, "high": 80}])
        llm = MagicMock()
        llm.bind_tools.return_value = llm
        llm.invoke.side_effect = [
            AIMessage(
        ...
        result = supervisor.run(config)
        # change_search_space は TrialRecord を生成しないが、エラーなく完了すること
    ...
    def test_subsequent_sobol_uses_expanded_space(
        ...
        # num_leaves を original high=200 より大きい 500 まで拡大
        expand_args = json.dumps([{"name": "num_leaves", "low": 20, "high": 500}])
        llm = MagicMock()
        llm.bind_tools.return_value = llm
        llm.invoke.side_effect = [
            AIMessage(
                ...
                tool_calls=[
                    {
                        "name": "change_search_space",
                        "args": {"param_updates": expand_args},
                        "id": "c1",
                    }
                ],
            ),
        ...
        result = supervisor.run(config)
        # current_param_space が拡大された空間になっていることを確認
        sobol_rows = result.trials_df[result.trials_df["tool_used"] == "sobol_search"]
        assert len(sobol_rows) > 0
        for _, row in sobol_rows.iterrows():
            assert 20 <= row["num_leaves"] <= 500

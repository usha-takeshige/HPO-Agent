...
- **change_search_space**: 探索空間を変更する（狭め・拡大どちらも可能）。有望な範囲が特定できた場合に
  絞り込み、狭めすぎた場合や新たな領域を探索したい場合に拡大する。以降の探索はこの空間を使用する。
  引数 param_updates に JSON 文字列で新しい範囲を指定する。
  例（絞り込み）: [{"name": "learning_rate", "low": 0.05, "high": 0.1}]
  例（拡大）: [{"name": "learning_rate", "low": 0.001, "high": 0.5}]
